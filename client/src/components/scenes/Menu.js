...
export default class Menu extends Phaser.Scene {  
  ...
  create() {
    ...
    this.physics.add.collider(this.player, this.asteroids, collisionDestroy, null, this)
    ...
    function shoot() {
    this.laser = this.physics.add.image(this.player.x, this.player.y,'laser').setScale(0.25)
    this.laser.setVelocityY(-900)
    this.physics.add.collider(this.laser, this.asteroids, collisionDestroy, null, this)
  }
    ...
    function collisionDestroy(collisionObject, asteroid){
      ...
      const explosion = this.add.sprite(asteroid.x, asteroid.y, "explosion").setScale(5)
